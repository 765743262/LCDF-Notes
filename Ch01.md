...
In general, the "." is called the _radix point, $A_{n-1}$ is referred to as the _most significant digit (msd)_ and $A_{-m}$ is called the _least significant digit (lsd)_.
...
The octal (base 8) and hexadecimal (base 6) systems are useful for representing binary quantities. **Each octal digit corresponds to three binary digits and each hexidecimal digit correspond to four binary digits.**

Table shows some converions:

![TABLE 1-3](Pics/1-3.png)

Note in hexidecimal system A, B, C, D, E, Frepresent 10, 11, 12, 13, 14, 15 respectively.
...
Multiplying 59F and E46 also needs to be perform with decimal.
...
Many applications of digital computers require the handling of dtat consisting not only of numbers, but also letters. It is necessary to formulate a binary code for any alphnumeric characters as well as many special characters.
...
Over the years, many different character sets were created to represent the scipts used in various languages, as well as special technical and methamatical symbols. These character sets were incompatible with each other.
...
To **detect erros in data communication and processing**, an additional bit is ometimes added to a **binary code word** to define its **parity**. 
