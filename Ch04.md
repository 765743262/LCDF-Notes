...
In the case of a signed number operation, $V = 1$ always implies that an overflow has occurred.

In the case of an unsigned number addition, $C = 1$ implies an overflow; if the operation is subtraction, $C = 1$ implies a correction step is needed.
...
A $n$-bit _incrementer_ performs $A + 1$ and can be obtained by using a binary adder with $B = 00..1$. **Since $B$ is constant, you can expect that the resulting circuit can be simplified.** By substuting $1$ or $0$ for $B$ in the equations, you can contract them.
...
This can be implemented with a adder-subtractor and contraction.
